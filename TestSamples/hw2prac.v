 ...
 *)
(*
 * (a) YOUR ANSWER HERE - n+1 (n S and 1 O)
 * (b) YOUR ANSWER HERE - n/2 even_SS and 1 even_O
 * (c) YOUR ANSWER HERE - If n is odd, then n/2 is not a valid count for number of even_SS used thus the proof can't exist.
 *)
...
Proof.
  ...
  induction H.
  - exists O. auto.
  - destruct IHeven as [d1 d2]. exists (S d1). simpl. auto. 
Qed.
...
Proof.
  ...
  induction H.
  - simpl. auto.
  - simpl. apply even_SS. exact IHeven.
Qed.
...
Lemma app_length_lemma:
    forall A (l1 : list A) (l2 : list A),
      length (l1 ++ l2) = (length l1) + (length l2).
Proof.
    intros.
    induction l1 as [| p lp IHlp].
    - reflexivity.
...
Proof.
  (* YOUR CODE HERE *)
  intros.
  induction l as [| x ls' IHls'].
  - reflexivity.
  - simpl. rewrite app_length_lemma. simpl. rewrite add_comm_again. simpl. rewrite IHls'. reflexivity. 
Qed.
...
Abort.
  ...
  (* This will not work since sum_list_tailrec is not a recursive function.
  Its a definition that calls another recursive function,
  so as per CSE 505 motto, this needs to be done by proving another lemma
  about the recursive function sum_list_tr *)


Lemma sum_list_tr_lemma:
    forall l acc,
        sum_list_tr l acc = sum_list l + acc.
Proof.
    intros l.
    induction l as [|p lp IHlp].
    - intros acc. reflexivity.
    - intros acc1.
      simpl.
      specialize (IHlp (p + acc1)).
      rewrite IHlp.
      lia.
Qed.
...
Proof.
  ...
  induction l as [|p lp IHlp].
  - reflexivity.
  - simpl. rewrite sum_list_app. simpl. rewrite IHlp. lia.
Qed.
